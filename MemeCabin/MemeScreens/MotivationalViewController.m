...
@interface MotivationalViewController ()
...
@implementation MotivationalViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [[GoogleAnalyticsCustom getSharedInstance] setCurrentScreenName:@"Motivational Screen"];
...
-(void)updateBottomBadgeCount
{
    ...
    if ((UIAppDelegate.currentBadgeMemeEveryOne+UIAppDelegate.currentBadgeMemeMotiInsp+UIAppDelegate.currentBadgeMemeRacy+UIAppDelegate.currentBadgeMemeSpiffyGif) > 0) {
        ...
        badgeForMotivationalMeme= [CustomBadge customBadgeWithString:badgeString
        ...
        [badgeForMotivationalMeme setFrame:
         CGRectMake(_backToHomeButton.frame.size.width-badgeForMotivationalMeme.frame.size.width/2,
                    -5,
                    badgeForMotivationalMeme.frame.size.width,
                    badgeForMotivationalMeme.frame.size.height)];
        [_backToHomeButton addSubview:badgeForMotivationalMeme];
    }
    else
        [badgeForMotivationalMeme removeFromSuperview];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        appdelegate.badgeMotivationalMeme = appdelegate.badgeMotivationalMeme;
        NSLog(@"if-appdelegate.badgeEveryoneMeme = %d",appdelegate.badgeMotivationalMeme);
    }
    else
    {
        /*
        ...
        NSLog(@"else-appdelegate.badgeEveryoneMeme = %d",appdelegate.badgeMotivationalMeme);*/
    ...
    NSLog(@"Motivational didSelect ID = %@",memePhoto.photoID);
    
    [[GoogleAnalyticsCustom getSharedInstance] clickEventViewController:@"Show Motivation Meme Details"];
    ...
    [swipeView setControllerIdentifier:2];
...
- (IBAction)markAallButtonAction:(UIButton *)sender
{
    ...
    else
    {
        for (MemePhoto *photo in UIAppDelegate.photoArray){
            ...
            UIAppDelegate.badgeMotivationalMeme = UIAppDelegate.badgeMotivationalMeme - 1;
        ...
        UIAppDelegate.currentBadgeMemeMotiInsp = 0;
        [[NSUserDefaults standardUserDefaults] setInteger:UIAppDelegate.currentBadgeMemeMotiInsp forKey:MEME_MOTI_INSP_CURRENT_COUNT];
        [[NSUserDefaults standardUserDefaults] synchronize];
        [UIAppDelegate setAllBadgesCountForApplication];
        [self updateBottomBadgeCount];
...
-(void)loadRefreshedImages
{
    ...
    NSString *urlString = MEME_MOTIVATIONAL;
                 ...
                 for (NSDictionary *listDic in listArray)
                 {
                     ...
                     if (!test) {
                         [dbhelper insertWithMemeID:photo.photoID
                                          likeCount:photo.photoLike withCategory:@"2"];
                     }
                     
                     
                     [UIAppDelegate.photoArray addObject:photo];
                     //NSLog(@"Motivational_server_PhotoID = %@",photo.photoID);
                 }
                 //userPhotoArray = [dbhelper getAllMemes];
                 //[_myCollectionView reloadData];
                 
                 NSLog(@"UIAppDelegate.UIAppDelegate.photoArray.count=%ld",(unsigned long)UIAppDelegate.photoArray.count);
                 [self refresh:nil];
...
{
    NSLog(@"MotivationalViewController dismissSlideMenu tapped!!");
