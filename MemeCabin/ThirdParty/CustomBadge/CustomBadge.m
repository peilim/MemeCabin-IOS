...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
	...
	CGSize stringSize = [badgeString sizeWithAttributes:
                         @{NSFontAttributeName:
                               [UIFont boldSystemFontOfSize:12.0f*scaleFactor]}];
    
    //NSLog(@"stringSize=%f",stringSize.width);
    
	CGFloat flexSpace;
	if ([badgeString length]>=2) {
		flexSpace = [badgeString length];
		rectWidth = 15 + (stringSize.width + flexSpace); rectHeight = 20;
		retValue = CGSizeMake(rectWidth*badgeScaleFactor*scaleFactor, rectHeight*badgeScaleFactor*scaleFactor);
	} else {
		retValue = CGSizeMake(20*badgeScaleFactor*scaleFactor, 20*badgeScaleFactor*scaleFactor);
	}
...
- (void)drawRect:(CGRect)rect {
	...
	if ([self.badgeText length]>0) {
		[badgeTextColor set];
		CGFloat sizeOfFont = 13.5*badgeScaleFactor*scaleFactor;
		if ([self.badgeText length]<2) {
			sizeOfFont += sizeOfFont*0.20;
		}
		UIFont *textFont =  [self fontForBadgeWithSize:sizeOfFont];
        
         NSDictionary *fontAttr = @{ NSFontAttributeName : textFont, NSForegroundColorAttributeName : badgeTextColor };
